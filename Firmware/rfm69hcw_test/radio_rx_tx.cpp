...
//#endif
  ...
  Serial.println("![Transceiving at 433 MHz...]!");
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      if (input == '\n') {
        if (line == "![YES_RETRY]!") {
          retry = true;
          Serial.println("![YES_RETRY]!");
        } else if (line == "![NO_RETRY]!") {
          retry = false;
          Serial.println("![NO_RETRY]!");
        } else {
          if (retry) {
            Serial.println("![SENDING W/ RETRY '" + line + "']!");
            line += '\n';
            radio.sendWithRetry(TRANSMIT_TO, line.c_str(), line.length());
          } else {
            Serial.println("![SENDING ONCE '" + line + "']!");
            line += '\n';
