...
//modified by neelay, 2019-01-23. this is deprecated.
//    _interrupt.rise(this, &RFM69::isr0);

    _interrupt.rise(callback(this, &RFM69::isr0));
...
void RFM69::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
    ...
    while (_interrupt == 0 && t.read_ms() - txStart < RF69_TX_LIMIT_MS) {
        //pc.printf("t: %d", t.read_ms() - txStart);
        pc.putc('w');
    } // wait for DIO0 to turn HIGH signalling transmission finish
    pc.printf("after interrupt while loop, %d\r\n", t.read_ms() - txStart);
